...
import pxb.android.dex2jar.v4.tree.ArrayIns;
import pxb.android.dex2jar.v4.tree.Base;
import pxb.android.dex2jar.v4.tree.NewArrayIns;
...
public class V4CodeAdapter implements DexCodeVisitor, Opcodes, DexOpcodes {
	 ...
	 * @param mv
	 */
	public V4CodeAdapter(Method method, MethodVisitor mv) {
		super();
		this.method = method;
		this.mv = mv;
	}
	...
	public void visitArrayInsn(int opcode, int regFromOrTo, int array, int index) {
		...
		try {
			if (opcode >= OP_APUT) {
				base.add(ArrayIns.put(ARRAY_TYPE_MAP[opcode - OP_APUT], array, index, regFromOrTo));
			} else {
				base.add(ArrayIns.get(ARRAY_TYPE_MAP[opcode - OP_AGET], array, index, regFromOrTo));
			}
		} catch (Exception e) {
	...
	public void visitArrayInsn(int opcode, String type, int saveToReg, int demReg) {
		...
		switch (opcode) {
		case OP_NEW_ARRAY:
			base.add(new NewArrayIns(Type.getType(type), demReg, saveToReg));
			break;
		default:
